...
$sql = "SELECT songs_id, title, artist,album, created_at 
        FROM songs
        ORDER BY created_at DESC LIMIT 12";
$result = $conn->query($sql);
$songs = $result ? $result->fetch_all(MYSQLI_ASSOC) : [];

// Spotlight = first 3 songs
$spotlight = array_slice($songs, 0, 3);

// Latest uploads = all returned songs
$latest = $songs;
?>
<!-- HERO SECTION -->
<section class="hero mb-5">
  <div class="row align-items-center">
    <div class="col-lg-7">
      ...
      <p class="lead mb-4">Dive into a curated collection of songs — from timeless classics to fresh drops.</p>

      <form class="search-bar d-flex align-items-center" action="search.php" method="get">
        <input type="search" name="q" class="form-control me-2" placeholder="Search by song or artist">
    ...
    <div class="col-lg-5 mt-4 mt-lg-0">
      <div class="card h-100 p-4">
        <div class="d-flex align-items-center mb-3">
          ...
          <small class="text-uppercase text-muted">Recently added</small>
        </div>
          ...
          <?php if (!empty($songs)): ?>
            <?php foreach (array_slice($songs, 0, 4) as $song): ?>
              <div class="d-flex justify-content-between align-items-center pb-2 border-bottom border-secondary-subtle">
                ...
                <a href="song.php?id=<?php echo (int)$song['songs_id']; ?>" class="btn btn-sm btn-outline-light">Play</a>
              </div>
            <?php endforeach; ?>
          <?php else: ?>
...
<section class="mb-5">
  <div class="d-flex justify-content-between align-items-center mb-3">
    ...
    <a href="search.php" class="btn btn-outline-light">View All</a>
  </div>
      ...
      <?php foreach ($spotlight as $song): ?>
        <div class="col-md-4">
          <div class="card h-100 p-4">
            ...
            <p class="text-muted">Sing along with the latest and most popular hits.</p>

            <a href="song.php?id=<?php echo (int)$song['songs_id']; ?>" class="btn btn-primary">🎧 View Lyrics</a>
          </div>
        </div>
      <?php endforeach; ?>
    <?php else: ?>
      <div class="col-12"><div class="alert alert-info">No spotlight songs available.</div></div>
    <?php endif; ?>
  </div>
</section>
        ...
        <div class="col-md-4">
          <div class="card h-100">
            <div class="card-body d-flex flex-column">
              ...
              <a href="song.php?id=<?php echo (int)$song['songs_id']; ?>" class="btn btn-primary mt-auto">View Lyrics</a>
            </div>
          </div>
        </div>
      <?php endforeach; ?>
    <?php else: ?>
      <div class="col-12"><div class="alert alert-info">No songs uploaded yet.</div></div>
    <?php endif; ?>
  </div>
</section>
