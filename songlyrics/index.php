...
$sql = "SELECT id, title, artist, created_at 
        FROM lyrics 
          ...
          <?php if (!empty($songs)): ?>
            <?php foreach (array_slice($songs, 0, 4) as $song): ?>
              <div class="d-flex justify-content-between align-items-center pb-2 border-bottom border-secondary-subtle">
                ...
                <a href="song.php?id=<?php echo (int)$song['id']; ?>" class="btn btn-sm btn-outline-light">Play</a>
              </div>
            <?php endforeach; ?>
          <?php else: ?>
      ...
      <?php foreach ($spotlight as $song): ?>
        <div class="col-md-4">
          <div class="card h-100 p-4">
            ...
            <a href="song.php?id=<?php echo (int)$song['id']; ?>" class="btn btn-primary">🎧 View Lyrics</a>
          </div>
        </div>
      <?php endforeach; ?>
    <?php else: ?>
